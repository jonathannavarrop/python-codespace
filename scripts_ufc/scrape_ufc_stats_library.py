...
def parse_fighter_tott(soup):
    ...
    # parse fighter record
    fighter_record = soup.find('span', class_='b-content__title-record').text
    ...
    for tag in tott.find_all('i'):
     ...
     # parse fighter's stats
    stats1 = soup.find_all('ul', class_='b-list__box-list b-list__box-list_margin-top')[0]
    # loop through each tag to get text and next_sibling text
    for tag in stats1.find_all('i'):
        # add text together and append to fighter_stats
        fighter_tott.append(tag.text + tag.next_sibling)

    stats2 = soup.find_all('ul', class_='b-list__box-list b-list__box-list_margin-top')[1]
    # loop through each tag to get text and next_sibling text
    for tag in stats2.find_all('i'):
        # add text together and append to fighter_stats
        fighter_tott.append(tag.text + tag.next_sibling)

    # clean each element in the list, removing '\n' and '  '
    fighter_tott = [text.replace('\n', '').replace('  ', '').split(':')[-1].strip() for text in fighter_tott if ':' in text]
